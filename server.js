...
(function() {
  // Dependencies
  var Card, Player, app, createNewDeck, deck, express, giveCards, http, io, path, players, server, shuffle, socketIO;
  ...
  giveCards = function(target, quantity, source) {
    target = source.slice(0, quantity);
  ...
  io.on('connection', function(socket) {
    ...
    return socket.on('new game', function() {
      ...
      for (i = 0, len = players.length; i < len; i++) {
        player = players[i];
        results.push(io.sockets.emit('state', players[socket.id].hand));
      }
      return results;
    });
  });
